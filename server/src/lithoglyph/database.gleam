...
pub fn open(state: DatabaseState) -> #(DatabaseState, client.FormDBResult(Nil)) {
  case state.conn {
    Some(_) -> #(state, Ok(Nil))
    None -> {
...
pub fn close(state: DatabaseState) -> #(DatabaseState, client.FormDBResult(Nil)) {
  case state.conn {
    None -> #(state, Ok(Nil))
    Some(conn) -> {
...
pub fn execute(
  state: DatabaseState,
  op: Operation,
) -> #(DatabaseState, client.FormDBResult(BitArray)) {
  case state.conn {
...
pub fn read(conn: Connection, op: Operation) -> client.FormDBResult(BitArray) {
  let encoded_op = encode_operation(op)
...
pub fn write(conn: Connection, op: Operation) -> client.FormDBResult(BitArray) {
  let encoded_op = encode_operation(op)
