...
pub fn main() {
  ...
  io.println("Connecting to FormDB...")
  let db = case client.connect(db_path) {
    Ok(conn) -> {
      io.println("Connected to FormDB successfully")
      conn
    }
    Error(e) -> {
      io.println("Warning: Failed to connect to FormDB: " <> format_error(e))
      io.println("Server will start but database operations will fail")
      io.println("Make sure FormDB NIF is loaded and database exists")
...
fn format_error(error: client.FormDBError) -> String {
  case error {
