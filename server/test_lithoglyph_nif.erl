...
%% Test FormDB Rust NIF from FormBase

main(_) ->
    io:format("~n=== FormDB NIF Test (FormBase) ===~n~n"),

    % Add paths
    true = code:add_patha("ebin"),
    true = code:add_patha("build/dev/erlang/formbase_server/ebin"),

    % Test 1: Version
    io:format("Test 1: Calling version()...~n"),
    Version = formdb_nif:version(),
    io:format("  ✓ Version: ~p~n~n", [Version]),

    % Test 2: Open database
    io:format("Test 2: Opening database...~n"),
    DbRef = formdb_nif:db_open(<<"/tmp/formbd_test">>),
    io:format("  ✓ Database opened~n~n", []),

    % Test 3: Begin transaction
    io:format("Test 3: Beginning transaction...~n"),
    {ok, TxnRef} = formdb_nif:txn_begin(DbRef, <<"read_write">>),
