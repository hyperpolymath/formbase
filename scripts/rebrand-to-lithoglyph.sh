...
# Renames FormDB/FormBD → Lithoglyph throughout codebase

set -euo pipefail

echo "=== Glyphbase Comprehensive Rebrand Script ==="
echo "FormDB/FormBD → Lithoglyph"
...
if [ -f "server/src/formbd.gleam" ]; then
    echo "  server/src/formbd.gleam → server/src/lithoglyph.gleam"
fi

if [ -f "server/src/formbd_nif.erl" ]; then
    echo "  server/src/formbd_nif.erl → server/src/lithoglyph_nif.erl"
fi

if [ -f "server/src/formdb_nif.erl" ]; then
    echo "  server/src/formdb_nif.erl → server/src/lithoglyph_nif.erl.old"
fi

if [ -f "server/native/src/formdb_nif.erl" ]; then
    echo "  server/native/src/formdb_nif.erl → server/native/src/lithoglyph_nif.erl"
fi

if [ -f "server/native/src/formdb_nif.zig" ]; then
    echo "  server/native/src/formdb_nif.zig → server/native/src/lithoglyph_nif.zig"
fi

if [ -f "server/test_formdb_nif.erl" ]; then
    echo "  server/test_formdb_nif.erl → server/test_lithoglyph_nif.erl"
fi
...
if [ -d "server/src/formdb" ]; then
    echo "  server/src/formdb/ → server/src/lithoglyph/"
fi
...
echo "5. Commit changes: git commit -am 'feat: rebrand FormDB/FormBD to Lithoglyph'"
